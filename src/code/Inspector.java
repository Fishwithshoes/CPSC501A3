...
public class Inspector {
	HashMap<Integer, Class<?>> recurseMap = new HashMap<Integer, Class<?>>();
	...
	public void displaySuperClass(Object currObject, Class<?> currClass, HashMap<Integer, Class<?>> currMap) {
	...
	public void recurseOnFieldObjects(Object currObject, boolean recursive, HashMap<Integer, Class<?>> currMap) {
		...
		for (int i = 0; i < fields.length; i++) {
			...
			if (currType.isPrimitive() == true){
				...
				try {
					arrayObject = fields[i].get(currObject);
					if (arrayObject != null) {
						inspect(arrayObject, recursive); //CHECK
